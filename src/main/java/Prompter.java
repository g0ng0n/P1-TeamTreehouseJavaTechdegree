...
class Prompter {
    ...
    public boolean promptForGuess() {
        ...
        try {
            ...
            if (this.jar.checkIfGuessOverTheMax(guessNumber)){
                System.out.println("Your guess must be less than maximum fill amounts");
            }else {
                isHit = this.jar.guessLogic(guessNumber);
                if (!isHit) {
                    String message = this.jar.checkHighLow(guessNumber);
